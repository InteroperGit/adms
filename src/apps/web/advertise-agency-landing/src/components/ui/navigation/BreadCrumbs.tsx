...
const pillBase =
  'inline-flex items-center rounded-full border px-3 py-1 text-xs font-medium sm:px-4 sm:py-1.5 sm:text-sm break-words';
    ...
    <div className="border-b border-border dark:border-gray-700 bg-background">
      <Container>
        <nav
          ...
          className="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-2 py-4"
        >
          {items.map((item, i) => {
            ...
            return (
              <span key={i} className="flex items-center gap-2">
                {i > 0 && (
                  <span
                    aria-hidden="true"
                    className={cn('shrink-0 mx-1', i > 0 ? 'pl-4 sm:pl-0' : '')}
                  >
