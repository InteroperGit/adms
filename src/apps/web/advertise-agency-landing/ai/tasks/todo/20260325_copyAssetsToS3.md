...
#!/usr/bin/env bash
set -euo pipefail
source .env.deploy  # contains ASSETS_S3_BUCKET
...
2. Add `ASSETS_S3_BUCKET=adms-assets` to `.env.deploy` (gitignored).
...
6. When CI is configured, add the pull step from the YAML above and store Beget credentials as GitHub Actions secrets (`BEGET_S3_ACCESS_KEY`, `BEGET_S3_SECRET_KEY`, `ASSETS_S3_BUCKET`).
...
| Risk | Mitigation |
|------|------------|
| Credentials in source control | `.env.deploy` and `~/.aws/credentials` are never committed |
