...
/**
 * TechnicalDesignOrderForm — форма для оформления заказа на техническое проектирование.
 * Этот компонент позволяет пользователю ввести все параметры, связанные с заказом на проектирование
 * (тип изделия, размеры, место установки, крепление, фото фасада, дополнительная информация, контактные данные).
 * Форма адаптивна и использует хуки состояния для управления данными формы и активными полями,
 * чтобы отображать пояснения к каждому полю.
 *
 * Основные особенности:
 * 1. В форме используются поля ввода для различных типов данных, включая текстовые поля
 * и текстовую область для комментариев.
 * 2. Для каждого поля предусмотрены обработчики событий `onChange`, `onFocus` и `onBlur`,
 * чтобы обновлять состояние формы и показывать пояснения.
 * 3. Компонент `FieldExplanation` динамически отображает описание для текущего активного поля.
 * 4. С помощью `useState` и `useEffect` обеспечивается корректная инициализация компонента и его состояние.
 * 5. Дизайн формы адаптирован для различных устройств с использованием CSS Grid и Tailwind CSS.
 *
 * Этот компонент подходит для сайтов, предлагающих услуги технического проектирования
 * с настройкой различных параметров изделия.
 */
